...
class PickerWidget extends StatefulWidget {
  ...
  const PickerWidget(
      {Key? key, required this.withImages, required this.withVideos, required this.onDone, required this.onCancel})
      : super(key: key);
...
class PickerWidgetState extends State<PickerWidget> {
  ...
  bool _loading = true;
  final MultiSelectorModel _selector = MultiSelectorModel();
  ...
  Widget build(BuildContext context) {
    return _loading ? const Center(child: CircularProgressIndicator()) : _buildWidget();
  }

  _buildWidget() {
    if (_albums.isEmpty) return const Center(child: Text("You have no folders to select from"));
